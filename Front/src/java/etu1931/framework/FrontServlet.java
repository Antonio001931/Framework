...
import etu1931.framework.AnnotationFonction;
import etu1931.framework.Mapping;
import etu1931.framework.ModelView;
import etu1931.framework.Parametre;
import etu1931.framework.UploadFile;
import etu1931.framework.Scope;
import etu1931.framework.Authentification;
import etu1931.framework.ApiRest;
        ...
        try {
            ...
            for (Class<?> c : annoted_classes) {
                ...
                for (Method m : methods) {
                    if (m.isAnnotationPresent(etu1931.framework.Url.class)) {
                        Mapping mapping = new Mapping();
                        mapping.setClassName(c.getName());
                        mapping.setMethod(m.getName());
                        etu1931.framework.Url app = m.getAnnotation(etu1931.framework.Url.class);
                        String url = app.nom();
        ...
        try {
            ...
            if (this.MappingUrls.containsKey(url)) {
                ...
                try {
                    ...
                    for (Field f : field) {
                        if (f.getType() == etu1931.framework.UploadFile.class) {
